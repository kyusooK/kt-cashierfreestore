<template>
    <div>
        <v-list two-line v-if="value.length > 0">
            <v-list-group v-model="isExpansion">
                <template v-slot:activator>
                    <v-list-item-content>
                        <v-list-item-title>AuthorityId</v-list-item-title>
                    </v-list-item-content>
                </template>
        ...
        <AuthorityId v-if="editMode && tick" offline :isNew="true" :inList="true"
...
<script>
    import AuthorityId from '../AuthorityId.vue';

    export default {
        name: 'AuthorityIdManager',
        components: {
            AuthorityId
        },
        props: {
